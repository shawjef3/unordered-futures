...
sealed trait CollectCondition {
  val maybeMax: Option[Int]
...
object CollectCondition {
  ...
  ) extends CollectCondition

  abstract case class Probability private (p: Double, override val maybeMax: Option[Int]) extends CollectCondition {
    def copy(
      p: Double = this.p,
  ...
  case object Never extends CollectCondition {
    override val maybeMax: Option[Int] = Some(0)

    override protected def checkMax(): Unit = ()
  }

  case object Always extends {
    override val maybeMax: Option[Int] = None
  } with CollectCondition

  case class OneEach(override val maybeMax: Option[Int] = None) extends CollectCondition

  abstract case class UpTo private (n: Int) extends {
    override val maybeMax: Option[Int] = Some(n)
  } with CollectCondition {
